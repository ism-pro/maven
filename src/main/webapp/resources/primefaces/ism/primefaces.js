...
PrimeFaces.dialog.DialogHandler = {openDialog: function (e) {
        ...
        c.on("load", function () {
            ...
            if (e.options.headerElement) {
                var m = PrimeFaces.escapeClientId(e.options.headerElement), j = c.contents().find(m);
                if (j.length) {
                    k = j;
                    n = true
                }
            ...
            if (!o.data("initialized")) {
                PrimeFaces.cw("DynamicDialog", h, {id: i, position: "center", sourceComponentId: e.sourceComponentId, sourceWidget: e.sourceWidget, onHide: function () {
...
PrimeFaces.widget.ContextMenu = PrimeFaces.widget.TieredMenu.extend({init: function (b) {
        ...
        this.jq.css({left: f, top: d, "z-index": ++PrimeFaces.zindex}).show();
        h.preventDefault();
        h.stopPropagation()
    }, hide: function () {
        var a = this;
...
PrimeFaces.widget.TreeTable = PrimeFaces.widget.DeferredWidget.extend({init: function (a) {
        ...
        var d = this, b = c.attr("data-rk"), a = {source: this.id, process: this.id, update: this.id, params: [{name: this.id + "_expand", value: b}], onsuccess: function (h, f, g) {
                PrimeFaces.ajax.Response.handle(h, f, g, {widget: d, handle: function (j) {
                        var i = c.next();
                        c.after(j);
