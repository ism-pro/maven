...
getter to populate <h:selectOneMenu> or <h:selectManyMenu>
    ...
    <html xmlns="http://www.w3.org/1999/xhtml"
        ...
        <ui:composition template="/tmpl/MainWindow.xhtml">
            ...
            <ui:define name="body">
                <h1><h:outputText value="${r"#{"}ism.${entityName}TitleEdit${r"}"}"/></h1>
...
<#list entityDescriptors as entityDescriptor>
                        <p:outputLabel value="${r"#{"}ism.${entityName}Field_${entityDescriptor.id?replace(".","_")}${r"}"}" <#if entityDescriptor.id?contains("Id")>rendered="true"</#if> for="${entityDescriptor.id?replace(".","_")}" />
                        <h:panelGrid columns="2" cellpadding="0px" cellspacing="0px" style="margin:0px; padding:0px;" <#if entityDescriptor.id?contains("Id")>rendered="true"</#if>>                
    <#if entityDescriptor.id?contains("Id")>
                            <p:outputLabel id="${entityDescriptor.id?replace(".","_")}"
                                           style="margin-left: 5px;"
                                           value="${r"#{"}${entityDescriptor.name}${r"}"}"  />
    <#elseif entityDescriptor.dateTimeFormat?? && entityDescriptor.dateTimeFormat != "">
                            <p:calendar id="${entityDescriptor.id?replace(".","_")}" pattern="dd/MM/yyyy" rendered="true"
                                        value="${r"#{"}${entityDescriptor.name}${r"}"}" 
    ...
    <#elseif entityDescriptor.blob>
                            <p:inputTextarea rows="4" cols="30" id="${entityDescriptor.id?replace(".","_")}" value="${r"#{"}${entityDescriptor.name}${r"}"}" 
                                             title="${r"#{"}ism.${entityName}FieldInfo_${entityDescriptor.id?replace(".","_")}${r"}"}"  <#if entityDescriptor.required>required="true" requiredMessage="${r"#{"}${bundle}.Create${entityName}RequiredMessage_${entityDescriptor.id?replace(".","_")}${r"}"}"</#if>/>
    ...
    <#else>
                            ...
                            <p:message for="${entityDescriptor.id?replace(".","_")}" <#if entityDescriptor.id?contains("Id")>rendered="false"</#if> />
                            </h:panelGrid>
                    ...
                    <h:panelGroup id="btn-crud-zone_id"  >
                        <p:commandButton icon="ui-btn-Save"
                                         styleClass="ui-btn"
                                         value="${r"#{"}ism.Change${r"}"}" 
                                         actionListener="${r"#{"}${managedBean}.update${r"}"}" 
