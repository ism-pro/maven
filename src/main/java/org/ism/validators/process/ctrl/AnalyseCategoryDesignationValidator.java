...
public class AnalyseCategoryDesignationValidator implements Validator, Serializable {
    ...
    public void validate(FacesContext fc, UIComponent uic, Object o) throws ValidatorException {
        ...
        List<AnalyseCategory> lst = analyseCategoryController.getItemsByDesignation(value, staffAuthController.getCompany());
        if (lst != null && !lst.isEmpty()) {
            if (input.getValue() != null) {
                if (lst.get(0).getAcDesignation().matches((String) input.getValue())) {
                    return;
                }
            }
