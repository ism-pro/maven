...
@ManagedBean
@SessionScoped
@FacesValidator(value = "docExplorerLinkValidator")
public class DocExplorerLinkValidator implements Validator, Serializable {

    public static final String P_DUPLICATION_SUMMARY_ID = "DocExplorerDuplicationLinkSummary";
    public static final String P_DUPLICATION_DETAIL_ID = "DocExplorerDuplicationLinkDetail";
    ...
    public void validate(FacesContext fc, UIComponent uic, Object o) throws ValidatorException {
        ...
        List<DocExplorer> lst = docExplorerController.getItemsByLink(value, staffAuthController.getCompany());
        if (lst != null && !lst.isEmpty()) {
            if (input.getValue() != null) {
                if (lst.get(0).getDcLink().matches((String) input.getValue())) {
                    return;
                }
            }
            FacesMessage facesMsg = JsfUtil.addErrorMessage(uic.getClientId(fc),
                    ResourceBundle.getBundle(JsfUtil.BUNDLE).
                            getString(P_DUPLICATION_SUMMARY_ID),
                    ResourceBundle.getBundle(JsfUtil.BUNDLE).
                            getString(P_DUPLICATION_DETAIL_ID)
                    + value);
