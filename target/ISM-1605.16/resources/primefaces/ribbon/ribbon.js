...
PrimeFaces.widget.Ribbon = PrimeFaces.widget.TabView.extend({
    ...
    bindEvents: function () {
        ...
        this.navContainer.children('li')
                .on('mouseover.ribbon', function (e) {
                    var element = $(this);
                    if (!element.hasClass('ui-state-disabled')) {
                        element.addClass('ui-state-hover');
                    }
                })
                .on('mouseout.ribbon', function (e) {
                    var element = $(this);
                    if (!element.hasClass('ui-state-disabled')) {
                        element.removeClass('ui-state-hover');
                    }
                })
                .on('click.ribbon', function (e) {
                    var element = $(this);
                    if ($(e.target).is(':not(.ui-icon-close)')) {
                        var index = element.index();
                        if (!element.hasClass('ui-state-disabled') && index !== $this.cfg.selected) {
                            $this.select(index);
                            element.trigger('focus.ribbon');
                        }
                    }
        ...
        if (this.cfg.scrollable) {
            this.navscroller.children('.ui-tabs-navscroller-btn')
                    .on('mouseover.ribbon', function () {
                        var el = $(this);
                        if (!el.hasClass('ui-state-disabled'))
                            $(this).addClass('ui-state-hover');
                    })
                    .on('mouseout.ribbon', function () {
                        var el = $(this);
                        if (!el.hasClass('ui-state-disabled'))
                            $(this).removeClass('ui-state-hover ui-state-active');
                    })
                    .on('mousedown.ribbon', function () {
                        var el = $(this);
                        if (!el.hasClass('ui-state-disabled'))
                            $(this).removeClass('ui-state-hover').addClass('ui-state-active');
                    })
                    .on('mouseup.ribbon', function () {
                        var el = $(this);
                        if (!el.hasClass('ui-state-disabled'))
                            $(this).addClass('ui-state-hover').removeClass('ui-state-active');
                    });
            this.navcrollerLeft.on('click.ribbon', function (e) {
                $this.scroll(100);
                e.preventDefault();
            });
            this.navcrollerRight.on('click.ribbon', function (e) {
                $this.scroll(-100);
                e.preventDefault();
            });
        }
    ...
    bindKeyEvents: function () {
        var $this = this;
        this.navContainer.children('li').on('focus.ribbon', function () {
                        ...
                        case keyCode.DOWN:
                            if ($this.focusedTabHeader) {
                                ...
                                if ($this.cfg.scrollable) {
                                    ...
                                    if (rightScroll) {
                                        $this.navcrollerRight.trigger('click.ribbon');
                                    }
                                }
                            }
                            e.preventDefault();
